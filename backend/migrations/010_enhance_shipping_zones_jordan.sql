-- Migration 010: Enhanced Shipping Zones for Jordan
...
-- 3. Add coordinates to existing areas table for distance calculation
ALTER TABLE `areas` 
ADD COLUMN `latitude` DECIMAL(10, 8) NULL AFTER `delivery_fee`,
ADD COLUMN `longitude` DECIMAL(11, 8) NULL AFTER `latitude`,
ADD COLUMN `zone_id` INT NULL AFTER `longitude`,
ADD KEY `fk_areas_zone` (`zone_id`),
ADD CONSTRAINT `fk_areas_zone` FOREIGN KEY (`zone_id`) REFERENCES `shipping_zones` (`id`) ON DELETE SET NULL;

-- 4. Update user_addresses table to include coordinates
...
ADD KEY `fk_address_shipping_zone` (`shipping_zone_id`),
ADD CONSTRAINT `fk_address_shipping_zone` FOREIGN KEY (`shipping_zone_id`) REFERENCES `shipping_zones` (`id`) ON DELETE SET NULL;
...
-- 7. Update existing areas with sample Jordan coordinates and zones
-- Amman areas
UPDATE `areas` SET 
  `latitude` = 31.9454, `longitude` = 35.9284, `zone_id` = 1 
WHERE `title_en` LIKE '%Amman%' OR `title_ar` LIKE '%عمان%' LIMIT 5;

-- Irbid areas  
UPDATE `areas` SET 
  `latitude` = 32.5556, `longitude` = 35.8500, `zone_id` = 2 
WHERE `title_en` LIKE '%Irbid%' OR `title_ar` LIKE '%إربد%' LIMIT 3;

-- Zarqa areas
UPDATE `areas` SET 
  `latitude` = 32.0727, `longitude` = 36.0888, `zone_id` = 2 
WHERE `title_en` LIKE '%Zarqa%' OR `title_ar` LIKE '%الزرقاء%' LIMIT 3;

-- 8. Add shipping preferences to app settings
INSERT IGNORE INTO `app_settings` (`setting_key`, `setting_value`, `description`) VALUES
('shipping_distance_unit', 'km', 'Unit for distance calculation (km or miles)'),
('shipping_calculation_method', 'zone_based', 'Method for shipping calculation: zone_based, area_based, or default'),
('shipping_max_distance', '100', 'Maximum delivery distance in km'),
('shipping_default_fee', '5.00', 'Default shipping fee when calculation fails'),
('shipping_google_maps_api_key', '', 'Google Maps API key for precise distance calculation'),
('shipping_enable_real_time_calc', '1', 'Enable real-time distance calculation using coordinates');

-- 9. Create indexes for better performance
CREATE INDEX `idx_zones_distance_range` ON `shipping_zones` (`min_distance_km`, `max_distance_km`, `is_active`);
CREATE INDEX `idx_areas_coordinates` ON `areas` (`latitude`, `longitude`);
CREATE INDEX `idx_addresses_coordinates` ON `user_addresses` (`latitude`, `longitude`);
CREATE INDEX `idx_branches_coordinates` ON `branches` (`latitude`, `longitude`);

-- 10. Add comments for documentation
ALTER TABLE `shipping_zones` COMMENT = 'Distance-based shipping zones with configurable pricing tiers';
ALTER TABLE `branch_shipping_zones` COMMENT = 'Branch-specific overrides for shipping zone pricing';
ALTER TABLE `shipping_calculations` COMMENT = 'Log of shipping cost calculations for analysis and debugging';
